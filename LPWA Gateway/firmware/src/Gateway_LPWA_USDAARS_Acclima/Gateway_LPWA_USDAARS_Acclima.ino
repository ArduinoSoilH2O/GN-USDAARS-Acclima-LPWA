...
/*Cellular Gateway - LPWA module
   ...
   **Version 2020.12.03 Adds lat and long to data and config strings, menu (for Steve only, not PSA)
   ...
   Version 2021.09.13 Remove disabling of watchdog timer
   Version 2021.12.20 Move uploadInterval into if statement (within menu), give 1 min for user
                      input for menu
                      Toggle recvMode depending on if GPRS_on is successful
   Version 2022.01.07 Move checking recvMode flag in various places 
    ...
    if (battV <= lowBatt) {     // Again, G will go to sleep if battV too low
      ...
      if (hrs == NISThr && mins == NISTmin) { // time to get time from NIST
        ...
        if(!forceRecv){
          ...
          gatewayInfo(); // moved to within sendDataSD
        }
      }
      setAlarm2();
    }
...
void gatewayInfo(){
  ...
  uint16_t gData_len = sizeof(VERSION) + sizeof(projectID) + sizeof(lat) + sizeof(lng) + 8 + bvLen + bTlen + pvcLen + pvLen + sizeof(timeSave);
  char gData[gData_len];                           // array for Gateway data, compiled below
  sprintf(gData, "%s~%s~%s,%s~%lu~%s~%s~%s~%s~%s", VERSION, projectID, serNum, bv, bT, pvc, pv, timeSave);
...
//  Serial.println(lng_len); 
  
  uint16_t gData_len = sizeof(VERSION) + sizeof(projectID) + lat_len + lng_len + 8 + bvLen + bTlen + pvcLen + pvLen + sizeof(timeSave);
  char gData[gData_len];                           // array for Gateway data, compiled below
  sprintf(gData, "%s~%s~%s,%s~%lu~%s~%s~%s~%s~%s", VERSION, projectID, lat, lng, serNum, bv, bT, pvc, pv, timeSave);
  delay(50);
//  Serial.println(gData);
 ...
 //--- Send data to Hologram
  ...
  if (gStatus == 1 && !recvMode) {    // added 05Jan22, without recvMode part G still tries to connect
    ...
    uint16_t gData_len = sizeof(VERSION) + sizeof(projectID) + lat_len + lng_len + 8 + bvLen + bTlen + pvcLen + pvLen + sizeof(timeSave);   // have to do it again, previous one is out of scope
//    Serial.println(gData_len);
    char gData[gData_len];                           // array for Gateway data, compiled below
    sprintf(gData, "%s~%s~%s,%s~%lu~%s~%s~%s~%s~%s", VERSION, projectID, lat, lng, serNum, bv, bT, pvc, pv, timeSave); delay(20);
    if (debug) Serial.println(gData);
//    Serial.println(gData[gData_len - 3],DEC);
//    Serial.println(gData[gData_len - 2],DEC);  
//    Serial.println(gData[gData_len - 1],DEC);
...
//    Serial.println(lng_len);
    ...
    uint16_t gData_len = sizeof(VERSION) + sizeof(projectID) + lat_len + lng_len + 8 + bvLen + bTlen + pvcLen + pvLen + sizeof(timeSave);   // have to do it again, previous one is out of scope
    char gData[gData_len];                           // array for Gateway data, compiled below
    sprintf(gData, "%s~%s~%s,%s~%lu~%s~%s~%s~%s~%s", VERSION, projectID, lat, lng, serNum, bv, bT, pvc, pv, timeSave); delay(100);
//       Serial.println(gData);
